...
void ovDrawString(u32 addr, u32 stride, u32 format, u32 scrnWidth, int posR, int posC, u32 r, u32 g, u32 b, u8* buf) {
    while(*buf) {
        if ((posR + 6 > 240) || (posC + 6 > scrnWidth)) {
            return;
        }
...
void ovDrawCharTiny(u32 addr, u32 stride, u32 format, u8 letter,int y, int x, u32 r, u32 g, u32 b){
...
void ovDrawStringTiny(u32 addr, u32 stride, u32 format, u32 scrnWidth, int posR, int posC, u32 r, u32 g, u32 b, u8* buf) {
    while(*buf) {
        if ((posR + 4 > 240) || (posC + 4 > scrnWidth)) {
            return;
        }
        ovDrawCharTiny(addr, stride, format, *buf, posR, posC, r, g, b);
        buf++;
        posC += 4;
